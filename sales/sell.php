               ...
               <div class="form-group">
                ...
                <select class="form-control select2 input-lg" style=" width: 100%;" id = 'stock'> 
                 ...
                 <?php foreach ($stocks as $stock): ?>
                  <option id = '<?=$stock['id']?>'><?= $stock['description']?></option>
                <?php endforeach ?>
              </select>
            </div>
            <div class="form-group">
  ...
  if ($('#stock').val() == '' || $('#quantity').val() == ''){
    ...
    $stock_id = $('option:selected',stock).attr('id');
        ...
        function(data){
          console.log(data);
          if(data.status == 'success'){
            $('#cart').append(getStockBox(data.description,data.quantity,data.subtotal,data.rmv_index));
            updateQuantityAndTotal();
